...
class TestWalktestStart(unittest.TestCase):
    """WalktestStart unit test stubs"""
    ...
    def make_instance(self, include_optional) -> WalktestStart:
        """Test WalktestStart
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `WalktestStart`
        """
        model = WalktestStart()
        if include_optional:
            return WalktestStart(
                cmd = 'STARTWALKTEST',
                included_points = 'ALL'
            )
        else:
