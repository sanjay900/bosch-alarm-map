...
class TestSupervisedConns(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> SupervisedConns:
        """Test SupervisedConns
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SupervisedConns`
        """
        model = SupervisedConns()
        if include_optional:
            return SupervisedConns(
                type = [
                    'IN.supervConn.1'
                    ],
                var_self = '',
