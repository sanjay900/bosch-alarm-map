...
class TestWalktestWtInner(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> WalktestWtInner:
        """Test WalktestWtInner
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `WalktestWtInner`
        """
        model = WalktestWtInner()
        if include_optional:
            return WalktestWtInner(
                area = '/1.1.Area.2.4',
                wt_status = 'PENDING'
            )
        else:
