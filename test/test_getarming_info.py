...
class TestGetarmingInfo(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> GetarmingInfo:
        """Test GetarmingInfo
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GetarmingInfo`
        """
        model = GetarmingInfo()
        if include_optional:
            return GetarmingInfo(
                cmd = 'ARMINGINFO'
            )
        else:
            return GetarmingInfo(
