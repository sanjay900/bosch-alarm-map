...
class TestBlocklockList(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> BlocklockList:
        """Test BlocklockList
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `BlocklockList`
        """
        model = BlocklockList()
        if include_optional:
            return BlocklockList(
                type = [
                    'IN.blocklockList.1'
                    ],
                var_self = '',
