...
class TestSupervisedIPC(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> SupervisedIPC:
        """Test SupervisedIPC
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SupervisedIPC`
        """
        model = SupervisedIPC()
        if include_optional:
            return SupervisedIPC(
                type = [
                    'IN.supervConn.1'
                    ],
                var_self = '',
