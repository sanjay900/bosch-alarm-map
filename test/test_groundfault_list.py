...
class TestGroundfaultList(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> GroundfaultList:
        """Test GroundfaultList
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GroundfaultList`
        """
        model = GroundfaultList()
        if include_optional:
            return GroundfaultList(
                type = [
                    'IN.gndFltList.1'
                    ],
                var_self = '',
