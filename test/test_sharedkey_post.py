...
class TestSharedkeyPost(unittest.TestCase):
    ...
    def make_instance(self, include_optional) -> SharedkeyPost:
        """Test SharedkeyPost
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SharedkeyPost`
        """
        model = SharedkeyPost()
        if include_optional:
            return SharedkeyPost(
                cmd = 'CREATE',
                checksum = '',
                encrypted = '',
                timestamp = ''
