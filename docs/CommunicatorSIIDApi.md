# bosch_alarm_map.CommunicatorSIIDApi

All URIs are relative to *https://169.254.10.10*

Method | HTTP request | Description
------------- | ------------- | -------------
[**getcommunicator_siid**](CommunicatorSIIDApi.md#getcommunicator_siid) | **GET** /{communicator_SIID} | Get information from a unique Communicator


# **getcommunicator_siid**
> Communicator getcommunicator_siid(communicator_siid)

Get information from a unique Communicator

The type communicator lists the communicator (AT2000, AT3000 or AT4000) and provides the
same interface as device type. It can be disabled.
...
with bosch_alarm_map.ApiClient(configuration) as api_client:
    ...
    api_instance = bosch_alarm_map.CommunicatorSIIDApi(api_client)
    communicator_siid = '/1.1.communicator.1.1' # str | Unique communicator SIID.

    try:
        # Get information from a unique Communicator
        api_response = api_instance.getcommunicator_siid(communicator_siid)
        print("The response of CommunicatorSIIDApi->getcommunicator_siid:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling CommunicatorSIIDApi->getcommunicator_siid: %s\n" % e)
```
...
------------- | ------------- | ------------- | -------------
 **communicator_siid** | **str**| Unique communicator SIID. | 
