...
------------- | ------------- | -------------
[**get_users**](UsersApi.md#get_users) | **GET** /users | Get a list with all user configs from system
[**post_users**](UsersApi.md#post_users) | **POST** /users | Activate or Deactivate a set of users on system
...
[Version:1.4.0032] The user list resource allows batched access to multiple or all users of the panel. With the URL it is not possible to read out all parameters of a user. To be able to read out all user parameters, the URL / usermodellist must be used. 
...
# **post_users**
> post_users(user_activate)

Activate or Deactivate a set of users on system

This resource models a set of MAP system users. A client can
also use this resource to activate or deactivate multiple users with a single request
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Activate or Deactivate a set of users on system
        api_instance.post_users(user_activate)
    except Exception as e:
