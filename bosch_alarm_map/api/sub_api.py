...
class SubApi:
    ...
    def get_sub(
        ...
        The MAP panel provides event notifications for all resources. This *GET* function is used, to get a subscription resource list. The subscription list resource allows access to all subscriptions. Furthermore, it is the location where a client can create a subscription.  ### Subscription Resource List: (/sub) ###  A central resource where a client can create a subscription. The location of the subscription resource is currently (“/sub”). However, a client shall not assume the location to be fixed but rather take the location from the panel description resource, to anticipate changes in future interface versions. The subscription resource also provides an overview on all existing subscriptions.  A client can decide which events to receive from the panel by setting the appropriate filters for the subscription. One client can also create multiple subscriptions so that the queue sizes and fetching behaviour fits to the expected occurrence of events. For example, it can be assumed that incidents will only occur rarely while device state changes will happen often. Thus, a client may create two subscriptions; one subscription for incidents only and one subscription for devices only. Thus, incidents can be prioritized and fetched quicker as if they would be part of the same subscription.  A client who wants to subscribe to all resources of the panel may specify a filter with all elements in the subscription list. Please note that the URL of the resources may change over time. Thus, a client shall look up the URL for the given resource type from the description resource to be forward compatible. 
        ...
        _param = self._get_sub_serialize(
    ...
    @validate_call
    def get_sub_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def post_sub(
        ...
        In order to create a subscription, a client has to send a POST request to the subscription list resource. The body of the POST request contains all relevant information about the subscription. In case the MAP panel accepts the subscription, it will respond with a 201 (\"Created\"). The response will contain further details about the subscription. The subscription will be created only for those resources to which the client has access rights to. Please note that the MAP panel may decide to enforce a different leaseTime and bufferSize as requested by the client. Thus, a client has to adhere to the leaseTime contained in the response and cannot assume that the originally requested time is going to be used for this subscription. A subscription may not be accepted due to three main reasons: - MAP panel is not able to accommodate more subscriptions (e.g. maximum number of subscribers exceeded) - Subscription request violates access rights - subscription request is malformed 

        :param create_sub: (required)
        :type create_sub: CreateSub
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._post_sub_serialize(
    ...
    @validate_call
    def post_sub_without_preload_content(
        self,
        create_sub: CreateSub,
        _request_timeout: Union[
            None,
