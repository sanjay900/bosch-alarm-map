...
class DaymodellistApi:
    ...
    def get_daymodellist(
        ...
        _param = self._get_daymodellist_serialize(
    ...
    @validate_call
    def get_daymodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    @validate_call
    def get_daymodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_daymodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
    ...
    @validate_call
    def postdaymodellist(
        self,
        day_modellist_post: Optional[DayModellistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necessary modifications to a client. It is designed to transport only changes of the daymodel attributes, which are related to a previous dayModelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically models are configured when the system is set up. Only minor adjustments are made afterwards.<br> A client transmits to a MAP system its last dayModelSyncID which it has stored. The MAP system responds a list that include all used IDs. Only the daymodels, where a modification happened between the received dayModelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other dayModels, only the dayModel ID without any other attributes is added. The client system takes the modifications from the body and can check for daymodels that are deleted, because deleted daymodels do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *dayModelSyncID* that is exactly the same as the dayModelSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *dayModelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative dayModelSyncID is sent by the client. - If the client sends a dayModelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest dayModelSyncID as an extra attribute named *dayModelModificationSyncID* into the RAM  when a dayModel was modified. A write to the existing database would break the existing datasheet structure and does not perform good enough. - By a (re-)boot the dayModelSyncID (URL /syncstatus) is increased by one and all *dayModelModificationSyncID* entries in the RAM with this increased *dayModelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the dayModelSyncID invalidates all dayModelSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param day_modellist_post:
        :type day_modellist_post: DayModellistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._postdaymodellist_serialize(
            day_modellist_post=day_modellist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def postdaymodellist_with_http_info(
        self,
        day_modellist_post: Optional[DayModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def postdaymodellist_without_preload_content(
        self,
        day_modellist_post: Optional[DayModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _postdaymodellist_serialize(
        self,
        day_modellist_post,
        _request_auth,
        _content_type,
        ...
        # process the form parameters
        # process the body parameter
        if day_modellist_post is not None:
            _body_params = day_modellist_post
