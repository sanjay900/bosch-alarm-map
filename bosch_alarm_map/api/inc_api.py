...
class IncApi:
    ...
    def get_inc(
        ...
        Alarms and troubles of the MAP system are modelled as so called incidents. In difference to other resources like area or point, an incident resource will not always exist, but will be created when an alarm/trouble appears in the system. The incident resource will also be removed from the REST-API when the alarm/trouble is resolved and disappears from the Keypad.  ### General Concept  The REST-API includes an event notification mechanism that allows spontaneous transmission of state changes (e.g. closing of a door contact) and occurrence of incidents (e.g. intrusion alarms). The notification mechanism uses a publish-subscribe pattern, where a client has to register with the MAP panel to specify which events it is interested in. This allows to custom tailor the events collected for each individual client. For example, a client may subscribe only for incidents in the system. In this case, it will only be notified about alarms and troubles but will not be notified about state changes of peripherals, significantly reducing the amount of events to be communicated. Once a subscription has been successfully created, the MAP panel will internally store the events for the client in a ring buffer with a defined size. It is the task of the client to fetch the events by issuing a request to a subscription specific URL.  #### Event delivery  The delivery of the events is done using a so called long polling mechanism. This means that an incoming request to fetch events will not return an answer until either a sufficient amount of events is available or a timer expires. A client specifies for each request how much events shall be included in the response as a minimum (minEvents), as a maximum (maxEvents) and how long the call is allowed to block at most (maxTime). In general, a client needs to fetch its events periodically to make sure that all events are received. The MAP panel will store only a limited amount of events for the client. In case events are not fetched in time, the oldest events in the buffer are overridden (ring buffer). A client can inspect the event id to recognize a buffer overflow. Even after a buffer overflow, MAP panel will continue to collect events. It is the choice of a client if an overflow is acceptable or not. If not, a client can unsubscribe to free the buffer and create a new subscription.  #### Lease time  A client also needs to ensure to poll for new events periodically, as the MAP panel will delete subscriptions that have been inactive for a given time (lease time negotiated during subscription). In cases where a client is not able to continuously poll events e.g. due to overload conditions, a client can fetch events with maxEvents set to 0. Such a request will be interpreted as subscription renewal but no events will be sent to the client.   #### Event structure  The system will create events in case of creation of a new resource, a state change of an existing resource or on deletion of a resource. Each event will have a unique ID, a timestamp which marks the time of occurrence, a type (created/changed/deleted) and the complete resource representation. In addition, a list of property keys is included in a state change event to indicate the properties that have changed in the resource.  The incident resource itself contains all relevant information about the alarm. In particular, it conveys who has acknowledged or handled the alarm. Creation, deletion and state changes of an incident are communicated via events. The incidents are located under the root resource “/inc”. This resource acts as an incident list, where all pending incidents in the system can be accessed. The individual incidents are located under /inc with the following structure: /inc/[Area ID]/[incident ID] 
        ...
        _param = self._get_inc_serialize(
    ...
    @validate_call
    def get_inc_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def post_inc(
        ...
        Depending on the device type and incident type, the life cycle of the incident varies. The interface provides information on what the client is expected to do. The “Handling Required” attribute of the incident resource indicates whether a user needs to handle/acknowledge the incident via the REST-API or the Keypad before it can get resolved and deleted. Usually incident can only be handled when area is disarmed. However there is a configuration option to explicitly allow handling in armed areas (“Allow Reset of Armed Areas”). Some types of incidents (e.g. LSN Loop tamper) require to be handled after the device is back to normal, thus “Handling Required” will remain true even if it was already handled. User has to retry handling until the incident is deleted. 

        :param incident_resource: (required)
        :type incident_resource: IncidentResource
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._post_inc_serialize(
    ...
    @validate_call
    def post_inc_without_preload_content(
        self,
        incident_resource: IncidentResource,
        _request_timeout: Union[
            None,
    ...
    def _post_inc_serialize(
        ...
        # process the form parameters
        # process the body parameter
        if incident_resource is not None:
            _body_params = incident_resource
