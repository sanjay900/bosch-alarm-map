...
class AccessmodellistApi:
    ...
    def get_accessmodellist(
        ...
        _param = self._get_accessmodellist_serialize(
    ...
    @validate_call
    def get_accessmodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    @validate_call
    def get_accessmodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_accessmodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
    ...
    @validate_call
    def postaccessmodellist(
        self,
        access_modelist_post: Optional[AccessModelistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necassary modifications to a client. It is designed to transport only changes of the accessmodel attributes, which are related to a previous accessModellSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically models are configured when the system is set up. Only minor adjustments are made afterwards.<br> A client transmits to a MAP system its last accessModellSyncID, which it has stored. The MAP system responds a list that included all used IDs. Only the accessModells, where a modification happened between the received accessModellSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other accessModells, only the accessModell ID without any other attributes is added. The client system takes the modifications from the body and can check for accessModells that are deleted, because deleted accessModells do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends an *accessModellSyncID* that is exactly the same as the accessModellSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends an *accessModellSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative accessModellSyncID is sent by the client. - If the client sends an accessModellSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest accessModellSyncID as an extra attribute named *accessModellSyncIDModificationSyncID* into the RAM  when an accessModell was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the accessModellSyncID (URL /syncstatus) is increased by one and all *accessModellSyncIDModificationSyncID* entries in the RAM with this increased *accessModellSyncID*. - If RPS for MAP updates the configuration on a MAP system,  this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the accessModellSyncID invalidates all accessModellSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param access_modelist_post:
        :type access_modelist_post: AccessModelistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._postaccessmodellist_serialize(
            access_modelist_post=access_modelist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def postaccessmodellist_with_http_info(
        self,
        access_modelist_post: Optional[AccessModelistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def postaccessmodellist_without_preload_content(
        self,
        access_modelist_post: Optional[AccessModelistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _postaccessmodellist_serialize(
        self,
        access_modelist_post,
        _request_auth,
        _content_type,
        _headers,
        _host_index,
        ...
        # process the form parameters
        # process the body parameter
        if access_modelist_post is not None:
            _body_params = access_modelist_post
