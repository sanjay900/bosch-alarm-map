...
class UsermodelApi:
    ...
    @validate_call
    def get_usermodel_by_id(
        self,
        id: Annotated[StrictStr, Field(description="Unique user ID of each MAP system user. Possible user IDs are in the range from 4 to 998.")],
        ...
        :param id: Unique user ID of each MAP system user. Possible user IDs are in the range from 4 to 998. (required)
        :type id: str
        ...
        _param = self._get_usermodel_by_id_serialize(
            id=id,
        ...
        _response_types_map: Dict[str, Optional[str]] = {
            '200': "GetUsermodelById200Response",
            '401': None,
            '403': None,
            '404': None,
            '500': None,
    ...
    @validate_call
    def get_usermodel_by_id_with_http_info(
        self,
        id: Annotated[StrictStr, Field(description="Unique user ID of each MAP system user. Possible user IDs are in the range from 4 to 998.")],
        _request_timeout: Union[
            None,
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[GetUsermodelById200Response]:
        """Get all parameters of specific user from MAP system
    ...
    @validate_call
    def get_usermodel_by_id_without_preload_content(
        self,
        id: Annotated[StrictStr, Field(description="Unique user ID of each MAP system user. Possible user IDs are in the range from 4 to 998.")],
        _request_timeout: Union[
            None,
    ...
    def _get_usermodel_by_id_serialize(
        self,
        id,
        _request_auth,
        _content_type,
    ...
    def post_usermodel(
        self,
        user_model_post: Optional[UserModelPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        This function is used to create, modify or delete users on a MAP system from the REST-API interface. It has the same functionalities like the RPS for MAP. This URL is designed to be used from a system, that controls remotely the user of several MAP systems. <br> To use the functions, every POST request needs a valid userModelSyncID. The initial userModelSyncID can be get from the commands GET /syncstatus or GET /usermodellist. After a successful operation, you get a new valid userModelSyncID back. It is recommended to use this ID, if you want to create, modify or delete several users.  ### Create User  To create a new user on a MAP system, the client must ensure that the key value *id* is not already used on the system. Further a valid userModelSyncID is required. The following examples shows how a user can be created:   - createUserWithMinimalParameters   - createUserWithSmartkeyProfile   - createUserFullConfig  ### Modify User  To modify an existing user on a MAP system, the client needs also a valid userModelSyncID. When modifying, all user parameters must be specified completely in the requested body schema. The example *modifyUser* shows how to modify a user.  ### Delete User  To delete an existing user on a MAP system, the client needs also a valid userModelSyncID. The example *deleteUser* shows how to delete a user.  ### Smartkey Profile  Smartkey Profile is an optional attribute set with up to 3 attributes: Name, Access Type, Token.  Name and Access Type must be specified together. 'Token Only' and 'Token And PINpad' Access Types require Token.  ### User Secrets Encryption  User attributes: passcode, oiiPassword, smartkeyToken are considered as user secrets.  User secrets are encrypted in GET output if shared key is available. Missing shared key will lead to plaintext output in GET.  User secrets encryption significantly affects performance of GET LIST, making it around 25% slower. POST and GET are slowed by a very small margin.  Shared Key change will trigger encryption on the first GET operation, making it slower by around a second for very large amount of secrets (~3000 secrets). For smaller databases encryption it will be proportionally faster.  In POST encryptedSecrets boolean attribute enables encrypted secret fields in POST payload. Encrypted data is mainly used for MAP panel synchronization, it is not intended for a consumer to encrypt secrets themselves.  If encryptedSecrets=true all secrets must be provided encrypted. If only one secret need to be modified in plain text, it is valid to omit the other secrets in the request.  In GET encryptedSecrets will be true in case user secrets are encrypted.  To have a successful decryption: Shared Key must be the same between device that encrypts and device that decrypts. Encrypted data cannot be shared between different userIDs, which means a valid encrypted secret for user 005 won't be accepted for any other userID. 

        :param user_model_post:
        :type user_model_post: UserModelPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._post_usermodel_serialize(
            user_model_post=user_model_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def post_usermodel_with_http_info(
        self,
        user_model_post: Optional[UserModelPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def post_usermodel_without_preload_content(
        self,
        user_model_post: Optional[UserModelPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _post_usermodel_serialize(
        ...
        # process the form parameters
        # process the body parameter
        if user_model_post is not None:
            _body_params = user_model_post
