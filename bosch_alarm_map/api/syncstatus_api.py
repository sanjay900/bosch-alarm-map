...
class SyncstatusApi:
    ...
    def get_syncstatus(
        ...
        Synchronization IDs are implemented to track changes in the MAP system database. Any change to corresponding model will increase its synchronization ID value by 1. System reboot will increase all synchronization IDs. Initial value of synchronization IDs is 0. Synchronization values are not directly modifiable and are managed internally.  ### Uptime & Restart counter  Uptime and restart counter help to properly manage Synchronization IDs.  ### Most frequent cases of /syncstatus usage detailed:  - SyncIDs did not change - Uptime has increased - Restart counter did not change - No changes were made to the MAP system. Synchronization is not required.  ### —  - SyncIDs have changed - Uptime has increased - Restart counter did not change - Changes were made to the MAP system, either partial GETMODIFIEDLIST or full synchronization is possible.  ### —  - Restart counter has changed or uptime has decreased - System was rebooted, full synchronization is required. GETMODIFIEDLIST output will be incomplete.   ### —  The IDs are only reset if the MAP database is deleted or overwritten. Possible cases are:  ### MAP software update  A system update is supplied with a default database. Synchronization IDs are always reset during a MAP panel firmware installation. MAP panel firmware can only be updated using RPS for MAP.  Recommended workflow is:  Backup MAP panel configuration using RPS for MAP. Update MAP panel firmware. Restore configuration from backup. Note: synchronization IDs are not part of a backup.  ### Failsafe mode  In the case of a critical system error, the system goes into the failsafe mode. This usually results in an alarm being sent directly to a higher-level reporting station, like a security office, a fire department or the police. Furthermore the MAP panel database is then replaced. To switch back from the failsafe mode to the normal operation state, a new installation of the current MAP panel firmware is required. This will install the default MAP database. 
        ...
        _param = self._get_syncstatus_serialize(
    ...
    @validate_call
    def get_syncstatus_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[SynchronizationIDs]:
        """Get all synchronization IDs
    ...
    @validate_call
    def get_syncstatus_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
