...
class IpchistoryApi:
    ...
    def get_ipchistory(
        ...
        IPCHistory (IP Communicator History) consists only of events related to reporting over IP. (IPCHistory is applicable only if the panel has been configured to report over IP. Events in IPCHistory will not be logged in MAP panel main history). A Client will have to fetch events from main history and IPCHistory separately IPC history has the same interface as history (i.e. history). In order to distinguish IPC and regular history, the history type ipcHistory is introduced. This type does not add any particular functionality. 
        ...
        _param = self._get_ipchistory_serialize(
    ...
    @validate_call
    def get_ipchistory_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    @validate_call
    def get_ipchistory_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
