...
class TimemodellistApi:
    ...
    def get_timemodellist(
        ...
        _param = self._get_timemodellist_serialize(
    ...
    @validate_call
    def get_timemodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    @validate_call
    def get_timemodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_timemodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
    ...
    @validate_call
    def posttimemodellist(
        self,
        time_modellist_post: Optional[TimeModellistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necessary modifications to a client. It is designed to transport only changes of the timemodel attributes, which are related to a previous timeModelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically models are configured when the system is set up. Only minor adjustments are made afterwards.<br> A client transmits to a MAP system its last timeModelSyncID, which it has stored. The MAP system responds a list that included all used IDs. Only the timemodels, where a modification happened between the received timeModelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other timemodels, only the timemodel ID without any other attributes is added. The client system takes the modifications from the body and can check for timemodels that are deleted, because deleted timemodels do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *timeModelSyncID* that is exactly the same as the timeModelSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *timeModelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative dtimeModelSyncID is sent by the client. - If the client sends a timeModelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest timeModelSyncID as an extra attribute named *timeModelModificationSyncID* into the RAM when a timeModel was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the timeModelSyncID (URL /syncstatus) is increased by one and all *timemodelModificationSyncID* entries in the RAM with this increased *timeModelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the timeModelSyncID invalidates all timemodelSyncID that are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param time_modellist_post:
        :type time_modellist_post: TimeModellistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._posttimemodellist_serialize(
            time_modellist_post=time_modellist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def posttimemodellist_with_http_info(
        self,
        time_modellist_post: Optional[TimeModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def posttimemodellist_without_preload_content(
        self,
        time_modellist_post: Optional[TimeModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _posttimemodellist_serialize(
        self,
        time_modellist_post,
        _request_auth,
        _content_type,
        ...
        # process the form parameters
        # process the body parameter
        if time_modellist_post is not None:
            _body_params = time_modellist_post
