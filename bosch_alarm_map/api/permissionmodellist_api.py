...
class PermissionmodellistApi:
    ...
    def get_permissionmodellist(
        ...
        _param = self._get_permissionmodellist_serialize(
    ...
    @validate_call
    def get_permissionmodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[PermissionModelList]:
        """Get all permission models of the MAP system
    ...
    @validate_call
    def get_permissionmodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_permissionmodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
        ...
        return self.api_client.param_serialize(
            method='GET',
            resource_path='/permissionmodellist',
    ...
    @validate_call
    def postpermissionmodellist(
        self,
        permission_modellist_post: Optional[PermissionModellistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necassary modifications to a client. It is designed to transport only changes of the permissionmodel attributes, which are related to a previous permissionmodelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically models are configured when the system is set up. Only minor adjustments are made afterwards<br> A client transmits to a MAP system its last permissionmodelSyncID, which it has stored. The MAP system response a list, that included all used ID's. Only the permissionmodels, where a modification happened between the received permissionmodelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other smartkeymodel, only the permissionmodel ID without any other attributes is added. The client system takes the modifications from the body and can check for permissionmodels that are deleted, because deleted permissionmodels do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *permissionmodelSyncID* that is exactly the same as the permissionmodelSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *permissionmodelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative permissionmodelSyncID is sent by the client. - If the client sends a permissionmodelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest permissionmodelSyncID as an extra attribute named *permissionmodelModificationSyncID* into the RAM when a permissionmodel was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the permissionmodelSyncID (URL /syncstatus) is increased by one and all *permissionmodelModificationSyncID* entries in the RAM with this increased *permissionmodelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the permissionmodelSyncID invalidates all permissionmodelSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param permission_modellist_post:
        :type permission_modellist_post: PermissionModellistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._postpermissionmodellist_serialize(
            permission_modellist_post=permission_modellist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def postpermissionmodellist_with_http_info(
        self,
        permission_modellist_post: Optional[PermissionModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def postpermissionmodellist_without_preload_content(
        self,
        permission_modellist_post: Optional[PermissionModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _postpermissionmodellist_serialize(
        self,
        permission_modellist_post,
        _request_auth,
        _content_type,
        _headers,
        _host_index,
        ...
        # process the form parameters
        # process the body parameter
        if permission_modellist_post is not None:
            _body_params = permission_modellist_post
