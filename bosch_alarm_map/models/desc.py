...
class Desc(BaseModel):
    ...
    base_url: Optional[StrictStr] = Field(default=None, description="Absolute URL to the REST-API server described by this resource", alias="baseURL")
    udn: Optional[StrictStr] = Field(default=None, description="Product ID of the MAP panel")
    friendly_name: Optional[StrictStr] = Field(default=None, description="Short description for the end user, e.g. “Intrusion Panel Room 127”", alias="friendlyName")
    firmware_version: Optional[StrictStr] = Field(default=None, description="The firmware version running on the MAP panel in format \"Major\".\"Minor\".\"Micro\"", alias="firmwareVersion")
    model_name: Optional[StrictStr] = Field(default=None, description="Identification of the MAP5000 panel; e.g. MAP5000, MAP5000-S, MAP5000-COM, MAP5000-SC", alias="modelName")
    profiles: Optional[List[StrictStr]] = Field(default=None, description="It contains one or more device profile Identifiers (e.g. “MAP5000.1.0”, “MAP5000.2.2”).")
    main_resources: Optional[List[DescMainResourcesInner]] = Field(default=None, description="A list of types and links for all main resources.", alias="mainResources")
    __properties: ClassVar[List[str]] = ["@type", "@self", "baseURL", "udn", "friendlyName", "firmwareVersion", "modelName", "profiles", "mainResources"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in main_resources (list)
        _items = []
        if self.main_resources:
            for _item_main_resources in self.main_resources:
                if _item_main_resources:
                    _items.append(_item_main_resources.to_dict())
            _dict['mainResources'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mainResources": [DescMainResourcesInner.from_dict(_item) for _item in obj["mainResources"]] if obj.get("mainResources") is not None else None
        })
        return _obj
