...
class InternalProgramIpArmingInfo(BaseModel):
    """
    InternalProgramIpArmingInfo
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="This command provides information about the reasons why the internal program cannot be activated. It provides the list of related device urls that prevent the internal program from being activated.", alias="@cmd")
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['IPARMINGINFO']):
            raise ValueError("must be one of enum values ('IPARMINGINFO')")
        return value
