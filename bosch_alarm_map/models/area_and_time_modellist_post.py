...
class AreaAndTimeModellistPost(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["@cmd", "areaAndTimeModelSyncID"]
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['GETMODIFIEDLIST']):
            raise ValueError("must be one of enum values ('GETMODIFIEDLIST')")
        return value
