...
class DeviceFwVersion(BaseModel):
    """
    DeviceFwVersion
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="The firmware version command retrieves the version of the firmware running on the device", alias="@cmd")
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['FWVERSION']):
            raise ValueError("must be one of enum values ('FWVERSION')")
        return value
