...
class WalktestStart(BaseModel):
    """
    This command is used to start the walktest.
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, alias="@cmd")
    included_points: Optional[StrictStr] = Field(default=None, description="Specifies which points should be put into test mode.", alias="includedPoints")
    __properties: ClassVar[List[str]] = ["@cmd", "includedPoints"]
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['STARTWALKTEST']):
            raise ValueError("must be one of enum values ('STARTWALKTEST')")
        return value

    @field_validator('included_points')
    def included_points_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ALL', 'SEISMICONLY']):
            raise ValueError("must be one of enum values ('ALL', 'SEISMICONLY')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@cmd": obj.get("@cmd"),
            "includedPoints": obj.get("includedPoints")
        })
        return _obj
