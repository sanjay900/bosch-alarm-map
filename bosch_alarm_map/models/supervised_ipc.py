...
class SupervisedIPC(BaseModel):
    ...
    incs: Optional[List[StrictStr]] = Field(default=None, description="A list of incidents that relate to the supervised connection.")
    path: Optional[StrictStr] = Field(default=None, description="Indicates the IPC communication path")
    __properties: ClassVar[List[str]] = ["@type", "@self", "enabled", "incs", "path"]
    ...
    def path_validate_enum(cls, value):
        ...
        if value not in set(['ethernet', 'wireless']):
            raise ValueError("must be one of enum values ('ethernet', 'wireless')")
        return value
