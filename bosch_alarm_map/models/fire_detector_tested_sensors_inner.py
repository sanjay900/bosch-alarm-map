...
class FireDetectorTestedSensorsInner(BaseModel):
    """
    ...
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default=None, description="When device is in walktest, the array contains objects with the possible types “o”, “t” or “c” and its combinations. The array is only filled while walktest is active. Otherwise it is empty (i.e. [])")
    tested: Optional[StrictBool] = Field(default=None, description="The tested field indicates whether the sensor has been tested successfully")
    __properties: ClassVar[List[str]] = ["type", "tested"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['o', 't', 'c']):
            raise ValueError("must be one of enum values ('o', 't', 'c')")
        return value
