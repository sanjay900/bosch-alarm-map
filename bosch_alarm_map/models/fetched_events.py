...
class FetchedEvents(BaseModel):
    """
    FetchedEvents
    """ # noqa: E501
    evts: Optional[List[Evts]] = Field(default=None, description="List of current subscriptions (respecting the access rights of the user)")
    __properties: ClassVar[List[str]] = ["evts"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in evts (list)
        _items = []
        if self.evts:
            for _item_evts in self.evts:
                if _item_evts:
                    _items.append(_item_evts.to_dict())
            _dict['evts'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "evts": [Evts.from_dict(_item) for _item in obj["evts"]] if obj.get("evts") is not None else None
        })
        return _obj
