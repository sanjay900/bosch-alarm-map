...
class PostTimemodelRequest(BaseModel):
    """
    PostTimemodelRequest
    """ # noqa: E501
    cmd: StrictStr = Field(alias="@cmd")
    time_model_id: StrictStr = Field(description="Unique Name of the Time model. The name is used to identify the item on the MAP System. The following characters are forbidden in identifier name: \" @ ; ", alias="timeModelID")
    reference_date: Optional[StrictStr] = Field(default=None, description="starting date of the Time model", alias="referenceDate")
    ignore_special_days: Optional[StrictBool] = Field(default=None, description="When set, the time model is active on special days. Special days are then ignored.", alias="ignoreSpecialDays")
    period_in_days: Optional[List[TimeModelPeriodInDaysInner]] = Field(default=None, description="The number of days in a Period for a Time Model.", alias="periodInDays")
    special_days: Optional[List[StrictStr]] = Field(default=None, description="List of Special Days.", alias="specialDays")
    ...
    __properties: ClassVar[List[str]] = ["@cmd", "timeModelID", "referenceDate", "ignoreSpecialDays", "periodInDays", "specialDays", "timeModelSyncID"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in period_in_days (list)
        _items = []
        if self.period_in_days:
            for _item_period_in_days in self.period_in_days:
                if _item_period_in_days:
                    _items.append(_item_period_in_days.to_dict())
            _dict['periodInDays'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "ignoreSpecialDays": obj.get("ignoreSpecialDays"),
            "periodInDays": [TimeModelPeriodInDaysInner.from_dict(_item) for _item in obj["periodInDays"]] if obj.get("periodInDays") is not None else None,
            "specialDays": obj.get("specialDays"),
