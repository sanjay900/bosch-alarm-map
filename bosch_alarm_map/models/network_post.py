...
class NetworkPost(BaseModel):
    ...
    NetworkPost
    """ # noqa: E501
    cmd: StrictStr = Field(description="Fixed String identifier for this command", alias="@cmd")
    ipv4_address: Optional[StrictStr] = Field(default=None, description="Define new IP address of the device", alias="ipv4Address")
    ipv4_netmask: Optional[StrictStr] = Field(default=None, description="Define new subnet mask used by the device", alias="ipv4Netmask")
    ipv4_gateway: Optional[StrictStr] = Field(default=None, description="Define new gateway used by the device", alias="ipv4Gateway")
    __properties: ClassVar[List[str]] = ["@cmd", "ipv4Address", "ipv4Netmask", "ipv4Gateway"]

    @field_validator('cmd')
    def cmd_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTIVATEDHCP', 'SETIPV4']):
            raise ValueError("must be one of enum values ('ACTIVATEDHCP', 'SETIPV4')")
        return value
