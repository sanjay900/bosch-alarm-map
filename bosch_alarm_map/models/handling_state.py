...
class HandlingState(BaseModel):
    """
    HandlingState
    """ # noqa: E501
    state: Optional[StrictStr] = Field(default=None, description="(MAP system currently supports only NONE and HANDLED)")
    user: Optional[StrictStr] = Field(default=None, description="The MAP system user that handled the incident")
    interface: Optional[StrictStr] = Field(default=None, description="The interface that was used to handle the incident (i.e. REST-API or Keypad)")
    __properties: ClassVar[List[str]] = ["state", "user", "interface"]
    ...
    def state_validate_enum(cls, value):
        ...
        if value not in set(['NONE', 'ACKED', 'HANDLED']):
            raise ValueError("must be one of enum values ('NONE', 'ACKED', 'HANDLED')")
        return value
