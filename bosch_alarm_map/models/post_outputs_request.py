...
POSTOUTPUTSREQUEST_ONE_OF_SCHEMAS = ["DeviceEnableDisable", "DeviceOnOff"]

class PostOutputsRequest(BaseModel):
    """
    PostOutputsRequest
    """
    # data type: DeviceEnableDisable
    oneof_schema_1_validator: Optional[DeviceEnableDisable] = None
    # data type: DeviceOnOff
    oneof_schema_2_validator: Optional[DeviceOnOff] = None
    actual_instance: Optional[Union[DeviceEnableDisable, DeviceOnOff]] = None
    one_of_schemas: Set[str] = { "DeviceEnableDisable", "DeviceOnOff" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PostOutputsRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, DeviceEnableDisable):
            ...
            match += 1
        # validate data type: DeviceOnOff
        if not isinstance(v, DeviceOnOff):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeviceOnOff`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PostOutputsRequest with oneOf schemas: DeviceEnableDisable, DeviceOnOff. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PostOutputsRequest with oneOf schemas: DeviceEnableDisable, DeviceOnOff. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into DeviceOnOff
        try:
            instance.actual_instance = DeviceOnOff.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PostOutputsRequest with oneOf schemas: DeviceEnableDisable, DeviceOnOff. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostOutputsRequest with oneOf schemas: DeviceEnableDisable, DeviceOnOff. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeviceEnableDisable, DeviceOnOff]]:
