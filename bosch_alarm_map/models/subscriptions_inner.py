...
class SubscriptionsInner(BaseModel):
    """
    SubscriptionsInner
    """ # noqa: E501
    urls: Optional[List[StrictStr]] = Field(default=None, description="List of links relative to the MAP system base URL from which events are to be received. The list may contain strings that follow the string matching as defined in *MAPOpenIntrusionInterface_ApplicationNotes.pdf* ,matching to reference multiple resources in a compact way.<br>  Examples: <br> Subscription to all events from all areas - [ \"*\" ]  Subscription to specific area ID - [ /1.1.Area.2.5 ] ")
    event_type: Optional[List[StrictStr]] = Field(default=None, description="Array of event types. Possible event types are: - CHANGED - CREATED - DELETED ", alias="eventType")
    __properties: ClassVar[List[str]] = ["urls", "eventType"]
