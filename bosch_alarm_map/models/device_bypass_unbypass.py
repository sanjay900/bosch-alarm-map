...
class DeviceBypassUnbypass(BaseModel):
    """
    DeviceBypassUnbypass
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="This operation initiates bypassing/unbypassing the device.  Bypassing a device will mean that only non-bypassable incidents will be generated from the device. Usually it is used to allow arming of an area even if the device is not normal e.g. an open window. ", alias="@cmd")
    __properties: ClassVar[List[str]] = ["@cmd"]
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['BYPASS', 'UNBYPASS']):
            raise ValueError("must be one of enum values ('BYPASS', 'UNBYPASS')")
        return value
