...
class PanelPost(BaseModel):
    ...
    PanelPost
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="This operation initiates restarting the panel or to get the panel support files. The functions with the support files is still a developer function. No guarantee that these develers all logs", alias="@cmd")
    persist_data: Optional[StrictBool] = Field(default=False, description="Indicates whether to persist persistent data or not, by default persistent data is purged", alias="persistData")
    __properties: ClassVar[List[str]] = ["@cmd", "persistData"]
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['RESTART']):
            raise ValueError("must be one of enum values ('RESTART')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@cmd": obj.get("@cmd"),
            "persistData": obj.get("persistData") if obj.get("persistData") is not None else False
        })
        return _obj
