...
class AreaConfiguration(BaseModel):
    ...
    AreaConfiguration
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description="The name used to identify the area.")
    siid: Optional[StrictStr] = Field(default=None, description="Auto generated unique Identifier. A, B, C and D are integer values. String would be “ControlPanelArea” or “Area”")
    disarm_if_all_disarmed: Optional[StrictStr] = Field(default=None, description="The control panel area (CPA) can only be disarmed if all other system areas have been disarmed. (Applicable only for Control Panel Area)", alias="disarmIfAllDisarmed")
    auto_disarmed: Optional[StrictBool] = Field(default=None, description="When all the areas in the system are disarmed, the Control Panel Area (CPA), if armed, will disarm. (Applicable only for Control Panel Area)", alias="autoDisarmed")
    parent_area_type: Optional[StrictStr] = Field(default=None, description="Defines the Parent Areas arming relationship with child areas.   - NOTAPARENT: Area contains no child devices. <br>   - MASTER: Area contains child area and its relationship is explained by “ChildAreaOptions” attribute.   - SHARED: Area arms automatically when all child areas are armed. Area disarms automatically when one of the child area is disarmed.   - BANK: Can be armed only after all child areas are armed.<br>   - PASSTHRU: Area arms automatically when one of the child areas is armed. Area disarms automatically when all child areas are disarmed. ", alias="parentAreaType")
    child_area_options: Optional[StrictStr] = Field(default=None, description="Configurable only for Master Area. <br>   - NONE: Not arming/disarming relationship with its child areas .   - ARMABLEIFCHILDARMED: Can be armed only if all child areas armed.   - ARMDISARMCHILDAREAS: Area will arm/disarm child areas when it is being armed/disarmed. ", alias="childAreaOptions")
    contained_area_list: Optional[List[StrictStr]] = Field(default=None, description="List of all child area(s) SIIDs.", alias="containedAreaList")
    device_list: Optional[List[StrictStr]] = Field(default=None, description="List of SIIDs of all devices configured in the area.", alias="deviceList")
    __properties: ClassVar[List[str]] = ["name", "siid", "disarmIfAllDisarmed", "autoDisarmed", "parentAreaType", "childAreaOptions", "containedAreaList", "deviceList"]

    @field_validator('disarm_if_all_disarmed')
    def disarm_if_all_disarmed_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['NOCONSTRAINT', 'YES']):
            raise ValueError("must be one of enum values ('NOCONSTRAINT', 'YES')")
        return value

    @field_validator('parent_area_type')
    def parent_area_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['NOTAPARENT', 'MASTER', 'SHARED', 'BANK', 'PASSTHRU']):
            raise ValueError("must be one of enum values ('NOTAPARENT', 'MASTER', 'SHARED', 'BANK', 'PASSTHRU')")
        return value

    @field_validator('child_area_options')
    def child_area_options_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['NONE', 'ARMABLEIFCHILDARMED', 'ARMDISARMCHILDAREAS']):
            raise ValueError("must be one of enum values ('NONE', 'ARMABLEIFCHILDARMED', 'ARMDISARMCHILDAREAS')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "siid": obj.get("siid"),
            "disarmIfAllDisarmed": obj.get("disarmIfAllDisarmed"),
            "autoDisarmed": obj.get("autoDisarmed"),
            "parentAreaType": obj.get("parentAreaType"),
            "childAreaOptions": obj.get("childAreaOptions"),
            "containedAreaList": obj.get("containedAreaList"),
            "deviceList": obj.get("deviceList")
        })
        return _obj
