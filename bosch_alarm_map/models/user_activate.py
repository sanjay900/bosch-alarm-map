...
class UserActivate(BaseModel):
    ...
    active_until: Optional[StrictStr] = Field(default=None, alias="activeUntil")
    __properties: ClassVar[List[str]] = ["@cmd", "activeUntil"]

    @field_validator('cmd')
    def cmd_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ACTIVATE', 'ACTIVATEUNTIL', 'DEACTIVATE']):
            raise ValueError("must be one of enum values ('ACTIVATE', 'ACTIVATEUNTIL', 'DEACTIVATE')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UserActivate from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UserActivate from a dict"""
        if obj is None:
            return None
