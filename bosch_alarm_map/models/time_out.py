...
class TimeOut(BaseModel):
    ...
    time_zone: Optional[StrictStr] = Field(default=None, description="Time zone following the IANA Time Zone Database (also known as Olson database) name ", alias="timeZone")
    date_time: Optional[StrictStr] = Field(default=None, description="Time and date in REST-API base specification compliant format including milliseconds. A client can extract local as well as UTC date time from this string", alias="dateTime")
    __properties: ClassVar[List[str]] = ["@type", "@self", "timeZone", "dateTime"]
