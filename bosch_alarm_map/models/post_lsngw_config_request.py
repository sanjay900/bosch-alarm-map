...
POSTLSNGWCONFIGREQUEST_ANY_OF_SCHEMAS = ["LSNGWConfigurationCreatePost", "LSNGWConfigurationDeletePost"]

class PostLSNGWConfigRequest(BaseModel):
    """
    PostLSNGWConfigRequest
    """

    # data type: LSNGWConfigurationCreatePost
    anyof_schema_1_validator: Optional[LSNGWConfigurationCreatePost] = None
    # data type: LSNGWConfigurationDeletePost
    anyof_schema_2_validator: Optional[LSNGWConfigurationDeletePost] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[LSNGWConfigurationCreatePost, LSNGWConfigurationDeletePost]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "LSNGWConfigurationCreatePost", "LSNGWConfigurationDeletePost" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PostLSNGWConfigRequest.model_construct()
        error_messages = []
        # validate data type: LSNGWConfigurationCreatePost
        if not isinstance(v, LSNGWConfigurationCreatePost):
            error_messages.append(f"Error! Input type `{type(v)}` is not `LSNGWConfigurationCreatePost`")
        else:
            return v

        # validate data type: LSNGWConfigurationDeletePost
        if not isinstance(v, LSNGWConfigurationDeletePost):
            error_messages.append(f"Error! Input type `{type(v)}` is not `LSNGWConfigurationDeletePost`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PostLSNGWConfigRequest with anyOf schemas: LSNGWConfigurationCreatePost, LSNGWConfigurationDeletePost. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[LSNGWConfigurationCreatePost] = None
        try:
            instance.actual_instance = LSNGWConfigurationCreatePost.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[LSNGWConfigurationDeletePost] = None
        try:
            instance.actual_instance = LSNGWConfigurationDeletePost.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostLSNGWConfigRequest with anyOf schemas: LSNGWConfigurationCreatePost, LSNGWConfigurationDeletePost. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], LSNGWConfigurationCreatePost, LSNGWConfigurationDeletePost]]:
