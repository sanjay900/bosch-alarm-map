...
class AreaArm(BaseModel):
    """
    This command is used to arm an area.
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="Indicates arming activity to be started", alias="@cmd")
    bypass_off_normal_devices: Optional[StrictBool] = Field(default=None, description="Bypass all devices that are off normal before arming.", alias="bypassOffNormalDevices")
    exit_delay: Optional[StrictStr] = Field(default=None, description="Defines whether the arming should happen without a delay (zero) with the user configured default exit delay or with the extended exit delay as configured for the area.", alias="exitDelay")
    __properties: ClassVar[List[str]] = ["@cmd", "bypassOffNormalDevices", "exitDelay"]
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['ARM']):
            raise ValueError("must be one of enum values ('ARM')")
        return value

    @field_validator('exit_delay')
    def exit_delay_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ZERO', 'USERDEFAULT', 'EXTENDED']):
            raise ValueError("must be one of enum values ('ZERO', 'USERDEFAULT', 'EXTENDED')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@cmd": obj.get("@cmd"),
            "bypassOffNormalDevices": obj.get("bypassOffNormalDevices"),
            "exitDelay": obj.get("exitDelay")
        })
        return _obj
