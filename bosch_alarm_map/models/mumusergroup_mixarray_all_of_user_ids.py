...
MUMUSERGROUPMIXARRAYALLOFUSERIDS_ONE_OF_SCHEMAS = ["List[int]", "int"]

class MumusergroupMixarrayAllOfUserIds(BaseModel):
    """
    ...
    # data type: List[int]
    oneof_schema_2_validator: Optional[Annotated[List[StrictInt], Field(min_length=2, max_length=2)]] = None
    actual_instance: Optional[Union[List[int], int]] = None
    one_of_schemas: Set[str] = { "List[int]", "int" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MumusergroupMixarrayAllOfUserIds.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MumusergroupMixarrayAllOfUserIds with oneOf schemas: List[int], int. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MumusergroupMixarrayAllOfUserIds with oneOf schemas: List[int], int. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MumusergroupMixarrayAllOfUserIds with oneOf schemas: List[int], int. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MumusergroupMixarrayAllOfUserIds with oneOf schemas: List[int], int. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[int], int]]:
