...
class Diagnose(BaseModel):
    ...
    Diagnose
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="Retrieves diagnostic information of the device.  Only walktestable device provides some diagnostic information that can be fetched using the diagnose command. (This information has not been included as a property of the resource as it changes only rarely and is usually only used in preparation of a walktest. Thus, moving it into a command allows to save bandwidth in most scenarios.) ", alias="@cmd")
    ...
    def cmd_validate_enum(cls, value):
        ...
        if value not in set(['DIAGNOSE']):
            raise ValueError("must be one of enum values ('DIAGNOSE')")
        return value
