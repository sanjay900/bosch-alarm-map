...
class InfrastructureDevice(BaseModel):
    """
    Device that is physically connected to the MAP.
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description="Device configured name. Usually related to device type, but can be changed to anything via RPS.")
    type: Optional[InfrastructureDeviceSIType] = None
    siid: Optional[StrictStr] = Field(default=None, description="Security Identifier (SI) ID that is used internally to communicate with the device.")
    properties: Optional[InfrastructureDeviceProperties] = None
    devices: Optional[List[InfrastructureDevice]] = Field(default=None, description="List of sub-devices physically connected to this device. Can be empty.")
    __properties: ClassVar[List[str]] = ["name", "type", "siid", "properties", "devices"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type"),
            "siid": obj.get("siid"),
            "properties": InfrastructureDeviceProperties.from_dict(obj["properties"]) if obj.get("properties") is not None else None,
            "devices": [InfrastructureDevice.from_dict(_item) for _item in obj["devices"]] if obj.get("devices") is not None else None
        })
        return _obj

# TODO: Rewrite to not use raise_errors
InfrastructureDevice.model_rebuild(raise_errors=False)
