...
POSTBATTERIESREQUEST_ONE_OF_SCHEMAS = ["DeviceBypassUnbypass", "DeviceEnableDisable"]

class PostBatteriesRequest(BaseModel):
    """
    PostBatteriesRequest
    """
    # data type: DeviceEnableDisable
    oneof_schema_1_validator: Optional[DeviceEnableDisable] = None
    # data type: DeviceBypassUnbypass
    oneof_schema_2_validator: Optional[DeviceBypassUnbypass] = None
    actual_instance: Optional[Union[DeviceBypassUnbypass, DeviceEnableDisable]] = None
    one_of_schemas: Set[str] = { "DeviceBypassUnbypass", "DeviceEnableDisable" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PostBatteriesRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, DeviceEnableDisable):
            ...
            match += 1
        # validate data type: DeviceBypassUnbypass
        if not isinstance(v, DeviceBypassUnbypass):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DeviceBypassUnbypass`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PostBatteriesRequest with oneOf schemas: DeviceBypassUnbypass, DeviceEnableDisable. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PostBatteriesRequest with oneOf schemas: DeviceBypassUnbypass, DeviceEnableDisable. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into DeviceBypassUnbypass
        try:
            instance.actual_instance = DeviceBypassUnbypass.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PostBatteriesRequest with oneOf schemas: DeviceBypassUnbypass, DeviceEnableDisable. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostBatteriesRequest with oneOf schemas: DeviceBypassUnbypass, DeviceEnableDisable. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeviceBypassUnbypass, DeviceEnableDisable]]:
