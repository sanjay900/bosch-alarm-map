...
class InfrastructureDeviceProperties(BaseModel):
    """
    List of configured properties of a device. Can be empty.
    """ # noqa: E501
    generic: Optional[List[InfrastructureDeviceGenericProperty]] = None
    rps: Optional[List[InfrastructureDeviceRPSProperty]] = None
    __properties: ClassVar[List[str]] = ["generic", "rps"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in generic (list)
        _items = []
        if self.generic:
            for _item_generic in self.generic:
                if _item_generic:
                    _items.append(_item_generic.to_dict())
            _dict['generic'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in rps (list)
        _items = []
        if self.rps:
            for _item_rps in self.rps:
                if _item_rps:
                    _items.append(_item_rps.to_dict())
            _dict['rps'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "generic": [InfrastructureDeviceGenericProperty.from_dict(_item) for _item in obj["generic"]] if obj.get("generic") is not None else None,
            "rps": [InfrastructureDeviceRPSProperty.from_dict(_item) for _item in obj["rps"]] if obj.get("rps") is not None else None
        })
        return _obj
