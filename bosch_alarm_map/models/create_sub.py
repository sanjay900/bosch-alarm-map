...
class CreateSub(BaseModel):
    ...
    CreateSub
    """ # noqa: E501
    cmd: Optional[StrictStr] = Field(default=None, description="Fixed String to identify this command", alias="@cmd")
    lease_time: Optional[StrictInt] = Field(default=None, description="Maximum number of seconds between two subscription renewal activities i.e. time between two event fetching requests encoded as JSON number. No fractions supported. The actual lease time is decided by the MAP panel.", alias="leaseTime")
    buffer_size: Optional[StrictInt] = Field(default=None, description="Requested number of events that will be stored by the MAP panel without overwriting an entry. The actual buffer size is decided by the MAP panel and provided in the response.", alias="bufferSize")
    subscriptions: Optional[List[List[SubscriptionsInner]]] = Field(default=None, description="Detailed information about the description in the same format as given during the subscription request")
    __properties: ClassVar[List[str]] = ["@cmd", "leaseTime", "bufferSize", "subscriptions"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in subscriptions (list of list)
        _items = []
        if self.subscriptions:
            for _item_subscriptions in self.subscriptions:
                if _item_subscriptions:
                    _items.append(
                         [_inner_item.to_dict() for _inner_item in _item_subscriptions if _inner_item is not None]
                    )
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "subscriptions": [
                    [SubscriptionsInner.from_dict(_inner_item) for _inner_item in _item]
