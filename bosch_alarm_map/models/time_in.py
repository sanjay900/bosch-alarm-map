...
class TimeIn(BaseModel):
    ...
    utc_date_time: StrictStr = Field(description="Date as defined above including timezone but in second precision. If no time zone is specified (e.g. +02:00 or Z), the request will be rejected. second fraction, e.g. millie seconds are not allowed. Actually this is supports a simple time synchronization that is not that accurate and what depends on the TCP transporting time.", alias="utcDateTime")
    __properties: ClassVar[List[str]] = ["@cmd", "utcDateTime"]

    @field_validator('cmd')
    def cmd_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SETTIME']):
            raise ValueError("must be one of enum values ('SETTIME')")
        return value
