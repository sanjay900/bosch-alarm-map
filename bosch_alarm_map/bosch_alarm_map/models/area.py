...
class Area(BaseModel):
    ...
    _alarm_observer: Observable = Observable()
    _ready_observer: Observable = Observable()
    _status_observer: Observable = Observable()
    type: Optional[List[StrictStr]] = Field(
        default=None, description="Fixed type identifier", alias="@type"
    )
    var_self: Optional[StrictStr] = Field(
        default=None, description="Link to the current resource", alias="@self"
    )
    armed: Optional[StrictBool] = Field(
        default=None, description="Indicates whether MAP panel is armed"
    )
    area_name: Optional[StrictStr] = Field(
        default=None, description="Name", alias="area name"
    )
    transitional_state: Optional[StrictStr] = Field(
        default=None,
        description="An empty JSON string (i.e. “”) indicates that area is not in a transitional state at the moment",
        alias="transitionalState",
    )
    oii_armable: Optional[StrictBool] = Field(
        default=None,
        description="True, if it is possible to disarm/arm the area over the REST-API interface. False, if Areas are configured only to be disarmed/armed blocklocks. False, if Area has relationships to Parent Area Type: Shared Area or Parent Area Type: Pass Thru Area. ",
        alias="oiiArmable",
    )
    ready_to_arm: Optional[StrictBool] = Field(
        default=None,
        description="If true, arming of the area is possible. Arming can either be done by a user using MAP system peripherals e.g. Keypad, Smartkey or via the REST-API, if area is “REST-API (OII) Armable”.",
        alias="readyToArm",
    )
    ready_to_disarm: Optional[StrictBool] = Field(
        default=None,
        description="If true, disarming of the area is possible. Disarming can either be done by a user using MAP system peripherals e.g. Keypad, Smartkey or via the REST-API, if area is “REST-API (OII) Armable”.",
        alias="readyToDisarm",
    )
    number_of_bypassed_devices: Optional[
        Annotated[int, Field(le=1500, strict=True, ge=0)]
    ] = Field(
        default=None,
        description="Number of devices that are bypassed in that area",
        alias="numberOfBypassedDevices",
    )
    walktest: Optional[StrictStr] = Field(
        default=None,
        description="If empty, then no walktest is running for this area. Otherwise this field contains the link to the active walktest relative to the server based URL.",
    )
    motion_detector_test_active: Optional[StrictBool] = Field(
        default=None,
        description="Indicates whether motion detector test is active",
        alias="motionDetectorTestActive",
    )
    chime_mode_active: Optional[StrictBool] = Field(
        default=None,
        description="Indicates whether chime mode is active",
        alias="chimeModeActive",
    )
    incs: Optional[List[StrictStr]] = Field(
        default=None,
        description="This field shows the relationship between incidents (alarm/trouble) and an individual area. Details about the incident are contained in the incident resource at its URL.",
    )
    ...
    @property
    def alarm_observer(self) -> Observable:
        """Get the alarm observer."""
        return self._alarm_observer

    @property
    def ready_observer(self) -> Observable:
        """Get the ready observer."""
        return self._ready_observer

    @property
    def status_observer(self) -> Observable:
