...
class SmartkeymodellistApi:
    ...
    def get_smartkeymodellist(
        ...
        _param = self._get_smartkeymodellist_serialize(
    ...
    @validate_call
    def get_smartkeymodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    @validate_call
    def get_smartkeymodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_smartkeymodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
        ...
        return self.api_client.param_serialize(
            method='GET',
            resource_path='/smartkeymodellist',
    ...
    @validate_call
    def post_smartkeymodellist(
        self,
        smartkey_model_list_post: Optional[SmartkeyModelListPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necassary modifications to a client. It is designed to transport only changes of the smartkeymodel attributes, which are related to a previous smartkeyModelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically models are configured when the system is set up. Only minor adjustments are made afterwards.<br> A client transmits to a MAP system its last smartkeyModelSyncID which it has stored. The MAP system responds a list that included all used IDs. Only the smartkeymodels, where a modification happened between the received smartkeyModelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other smartkeymodels, only the -Model ID without any other attributes is added. The client system takes the modifications from the body and can check for smartkeymodels that are deleted, because deleted smartkeymodels do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *smartkeyModelSyncID* that is exactly the same as the smartkeyModelSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *smartkeyModelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative smartkeyModelSyncID is sent by the client. - If the client sends a smartkeyModelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest smartkeyModelSyncID as an extra attribute named *smartkeyModelSyncIDModificationSyncID* into the RAM when a smartkeymodel was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the smartkeyModelSyncID (URL /syncstatus) is increased by one and all *smartkeyModelSyncIDModificationSyncID* entries in the RAM with this increased *smartkeyModelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the smartkeyModelSyncID invalidates all smartkeyModelSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param smartkey_model_list_post:
        :type smartkey_model_list_post: SmartkeyModelListPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._post_smartkeymodellist_serialize(
            smartkey_model_list_post=smartkey_model_list_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def post_smartkeymodellist_with_http_info(
        self,
        smartkey_model_list_post: Optional[SmartkeyModelListPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def post_smartkeymodellist_without_preload_content(
        self,
        smartkey_model_list_post: Optional[SmartkeyModelListPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _post_smartkeymodellist_serialize(
        self,
        smartkey_model_list_post,
        _request_auth,
        _content_type,
        _headers,
        _host_index,
        ...
        # process the form parameters
        # process the body parameter
        if smartkey_model_list_post is not None:
            _body_params = smartkey_model_list_post
