...
class TimeApi:
    ...
    def get_time(
        ...
        The time resource provides information on the current system time of the MAP panel including time zone. A client can also use this resource to set the time of the MAP panel. The time is given in precision of milliseconds. 
    ...
    def post_time(
        ...
        It is possible to set the date and time of the panel over the REST-API. This can be used for commissioning purposes or for time synchronization, where a client sets the time in regular intervals to assure a desired synchronization of its internal clock to the clock of the MAP panel. Currently the time zone is only configurable via RPSforMAP and cannot be set via the REST-API. This is considered to be appropriate, as the MAP system needs to be configured over RPS and the time zone should not change over time, as the installation location of the MAP panel is fixed. The time that is provided in the request is interpreted as UTC time. This means, even if the date time is including time zone information, for example 2014-07-20T16:11:42+02:00, the actual UTC time is calculated from that, for example 2014-07-20T14:11:42Z and the internal clock’s UTC is set accordingly. When fetching the time using a GET, the local time reflecting the configured time zone will be provided as described in get /time. 

        :param time_in: (required)
        :type time_in: TimeIn
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
        ...
        _param = self._post_time_serialize(
            time_in=time_in,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
    ...
    @validate_call
    def post_time_without_preload_content(
        self,
        time_in: TimeIn,
        _request_timeout: Union[
            None,
