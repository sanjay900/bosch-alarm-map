...
class UsermodellistApi:
    ...
    @validate_call
    def get_usermodellist(
        self,
        var_property: Annotated[Optional[StrictStr], Field(description="Querying by property parameter will filter all existing users with UserID that have specified property flag set. UserID property is a set of flags for each UserID.  List of properties: - MUM: Multi-User-MAP: reserved for MUM usage; MAP Keypad cannot use (create) Users with MUM flagged UserID.  Each UserID has exactly one set of flags attached, UserID can be set in range [4, 998]. ")] = None,
        ...
        This function returns a list of all users saved in the MAP panel database, including all user attributes. Furthermore, with the help of this URL it is possible to display only the users that are managed via a multi-user system. To use this function, the query parameter *property* must be set with the variable *MUM*. To assign to a user the property *MUM*, see the URL */mumusergroup*  .<br>  ### Example query URL to get all users with the MUM flag:   /usermodellist?property=MUM 

        :param var_property: Querying by property parameter will filter all existing users with UserID that have specified property flag set. UserID property is a set of flags for each UserID.  List of properties: - MUM: Multi-User-MAP: reserved for MUM usage; MAP Keypad cannot use (create) Users with MUM flagged UserID.  Each UserID has exactly one set of flags attached, UserID can be set in range [4, 998]. 
        ...
        _param = self._get_usermodellist_serialize(
    ...
    @validate_call
    def get_usermodellist_with_http_info(
        self,
        var_property: Annotated[Optional[StrictStr], Field(description="Querying by property parameter will filter all existing users with UserID that have specified property flag set. UserID property is a set of flags for each UserID.  List of properties: - MUM: Multi-User-MAP: reserved for MUM usage; MAP Keypad cannot use (create) Users with MUM flagged UserID.  Each UserID has exactly one set of flags attached, UserID can be set in range [4, 998]. ")] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def get_usermodellist_without_preload_content(
        self,
        var_property: Annotated[Optional[StrictStr], Field(description="Querying by property parameter will filter all existing users with UserID that have specified property flag set. UserID property is a set of flags for each UserID.  List of properties: - MUM: Multi-User-MAP: reserved for MUM usage; MAP Keypad cannot use (create) Users with MUM flagged UserID.  Each UserID has exactly one set of flags attached, UserID can be set in range [4, 998]. ")] = None,
        _request_timeout: Union[
            None,
    ...
    def _get_usermodellist_serialize(
        self,
        var_property,
        _request_auth,
        _content_type,
    ...
    @validate_call
    def post_usermodellist(
        self,
        user_modellist_post: Optional[UserModellistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necessary modifications to a client. It is designed to transport only changes of the user attributes, which are related to a previous userModelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically only a few modifications of a few users are done. Such as 1 of 200 users changed the password locally from a MAP Keypad.<br> A client transmits to a MAP system its last userModelSyncID, which it has stored. The MAP system responds a list that includes all used user IDs. Only the users, where a modification happened between the received userModelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other users, only the user ID without any other attributes is added. The client system takes the modifications from the body and can check for users that are deleted, because deleted users do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *userModelSyncID* that is exactly the same as the userModelSyncID from the database, only the userIDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *userModelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative userModelSyncID is sent by the client. - If the client sends a userModelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest userModelSyncID as an extra attribute named *userModificationSyncID* into the RAM  when a user was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the userModelSyncID (URL /syncstatus) is increased by one and all *userModificationSyncID*  entries in the RAM with this increased *userModelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the userModelSyncID invalidates all userModelSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param user_modellist_post:
        :type user_modellist_post: UserModellistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._post_usermodellist_serialize(
            user_modellist_post=user_modellist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def post_usermodellist_with_http_info(
        self,
        user_modellist_post: Optional[UserModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def post_usermodellist_without_preload_content(
        self,
        user_modellist_post: Optional[UserModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _post_usermodellist_serialize(
        self,
        user_modellist_post,
        _request_auth,
        _content_type,
        ...
        # process the form parameters
        # process the body parameter
        if user_modellist_post is not None:
            _body_params = user_modellist_post
