...
class SpecialdaymodellistApi:
    ...
    def get_specialdaymodellist(
        ...
        _param = self._get_specialdaymodellist_serialize(
    ...
    @validate_call
    def get_specialdaymodellist_with_http_info(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[SpecialDayModelList]:
        """Get all special day models of the MAP system
    ...
    @validate_call
    def get_specialdaymodellist_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
    ...
    def _get_specialdaymodellist_serialize(
        self,
        _request_auth,
        _content_type,
        _headers,
        ...
        return self.api_client.param_serialize(
            method='GET',
            resource_path='/specialdaymodellist',
    ...
    @validate_call
    def postspecialdaymodellist(
        self,
        special_day_modellist_post: Optional[SpecialDayModellistPost] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        The POST function is used to send only necassary modifications to a client. It is designed to transport only changes of the specialdaymodel attributes, which are related to a previous specialdayModelSyncID. This will improve the performance and reduce the network load significantly for the normal use cases. <br> Normal case means, that typically the specialdaymodels are created setting up the system. Normally only minor adjustments are made afterwards<br> A client transmits to a MAP system its last specialdayModelSyncID, which it has stored. The MAP system response a list, that included all used ID's. Only the specialdaymodels, where a modification happened between the received specialdayModelSyncID from the client and the current one of the MAP system will be sent with all user attributes. For all the other specialdayModels, only the specialdayModel ID without any other attributes is added. The client system takes the modifications from the body and can check for specialdaymodels that are deleted, because deleted specialdaymodels do not appear anymore in the response body.<br>  ### Remarks:  - If the client sends a *specialdayModelSyncID* that is exactly the same as the specialdayModelSyncID from the database, only the IDs without attributes are sent in the response, as there is no modification to be reported. - If the client sends a *specialdayModelSyncID* that is higher than the ID in the database of the MAP system, an error is returned. The same applies when a negative specialdayModelSyncID is sent by the client. - If the client sends a specialdayModelSyncID that is exactly 0, the response will contain all information fully. - The MAP system saves always the latest specialdayModelSyncID as an extra attribute named *specialdayModelModificationSyncID* into the RAM when a specialdayModel was modified. A write to the existing database would break the existing database structure and does not perform good enough. - By a (re-)boot the specialdayModelSyncID (URL /syncstatus) is increased by one and all *specialdayModelModificationSyncID* entries in the RAM with this increased *specialdayModelSyncID*. - If RPS for MAP updates the configuration on a MAP system, this will typically also cause a reboot. It is fine and the syncIDs are increased, to be sure a fully synchronization is processed from all REST-API clients. - The increase of the specialdayModelSyncID invalidates all specialdayModelSyncID which are saved by all REST-API clients and ensures all data will be synchronized if a reboot happens. 

        :param special_day_modellist_post:
        :type special_day_modellist_post: SpecialDayModellistPost
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        ...
        _param = self._postspecialdaymodellist_serialize(
            special_day_modellist_post=special_day_modellist_post,
            _request_auth=_request_auth,
            _content_type=_content_type,
            _headers=_headers,
            _host_index=_host_index
    ...
    @validate_call
    def postspecialdaymodellist_with_http_info(
        self,
        special_day_modellist_post: Optional[SpecialDayModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    @validate_call
    def postspecialdaymodellist_without_preload_content(
        self,
        special_day_modellist_post: Optional[SpecialDayModellistPost] = None,
        _request_timeout: Union[
            None,
    ...
    def _postspecialdaymodellist_serialize(
        self,
        special_day_modellist_post,
        _request_auth,
        _content_type,
        _headers,
        _host_index,
        ...
        # process the form parameters
        # process the body parameter
        if special_day_modellist_post is not None:
            _body_params = special_day_modellist_post
