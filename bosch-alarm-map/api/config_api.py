...
class ConfigApi:
    ...
    def get_confic(
        ...
        A configuration file is provided that describes the areas, internal programs, devices and their relationship. A client can download the configuration on start up to understand which items are configured in the MAP system. In particular, the relationship between areas and devices as well as among devices is given in this resource. The relationship information is only given with the configuration. The resources themselves do not have links between each other, as the relationships cannot change during runtime. <br> **Object Structure** <br> The configuration object has three major parts. <br> 1. Configured areas <br> In the first part of the configuration the list of all configured areas and their properties is provided. In addition, each area contains a list of all devices that are part of this area. <br> 2. Internal programs <br> The second part of the configuration is the internal program configuration which provides list of configured internal programs and the devices that are configured for these internal programs. <br> 3. Device configuration <br> The third part of the configuration is the device configuration. This part includes details about the devices. 
        ...
        _param = self._get_confic_serialize(
    ...
    @validate_call
    def get_confic_without_preload_content(
        self,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
