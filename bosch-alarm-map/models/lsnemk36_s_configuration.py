...
class LSNEMK36SConfiguration(BaseModel):
    """
    LSNEMK36SConfiguration
    """ # noqa: E501
    type: StrictStr = Field(description="Type of a device.")
    name: StrictStr = Field(description="LSN device internal name.")
    area: StrictStr = Field(description="LSN device area.")
    point_type: StrictStr = Field(description="Point device type.", alias="pointType")
    supports_delayed_reporting: Optional[StrictBool] = Field(default=None, description="Delayed reporting support.", alias="supportsDelayedReporting")
    walktest_category: Optional[LSNWalktestCategoryProperty] = Field(default=None, alias="walktestCategory")
    chime_mode_capable: Optional[StrictBool] = Field(default=None, description="Controls whether the point can trigger the chime tone on Keypad.", alias="chimeModeCapable")
    delay_for_alarm_detection: Optional[Annotated[int, Field(le=255, strict=True, ge=0)]] = Field(default=None, description="Delay for alarm detection in seconds.", alias="delayForAlarmDetection")
    entry_point: Optional[StrictStr] = Field(default=None, description="Device entry point.", alias="entryPoint")
    exit_point: Optional[StrictStr] = Field(default=None, description="Device exit point.", alias="exitPoint")
    walktest_trigger_frequency: Optional[LSNWalktestTriggerFrequencyProperty] = Field(default=None, alias="walktestTriggerFrequency")
    reporting_number: Optional[Annotated[int, Field(le=1500, strict=True, ge=1)]] = Field(default=None, description="LSN device reporting number.", alias="reportingNumber")
    ignore_magnetic_supervision: Optional[StrictBool] = Field(default=None, description="Ignore tamper condition.", alias="ignoreMagneticSupervision")
    __properties: ClassVar[List[str]] = ["type", "name", "area", "pointType", "supportsDelayedReporting", "walktestCategory", "chimeModeCapable", "delayForAlarmDetection", "entryPoint", "exitPoint", "walktestTriggerFrequency", "reportingNumber", "ignoreMagneticSupervision"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['EMK36', 'AMK4', 'MCx2', 'RSC2', 'EMK36S', 'AMK4S', 'MCx3', 'RSC3', 'SKI100', 'SKA100', 'StdIntr50', 'StdIntr51', 'StdIntr52', 'StdIntr53', 'StdIntr54', 'StdIntr55', 'StdIntr56', 'ND100', 'ND200', 'UP370T']):
            raise ValueError("must be one of enum values ('EMK36', 'AMK4', 'MCx2', 'RSC2', 'EMK36S', 'AMK4S', 'MCx3', 'RSC3', 'SKI100', 'SKA100', 'StdIntr50', 'StdIntr51', 'StdIntr52', 'StdIntr53', 'StdIntr54', 'StdIntr55', 'StdIntr56', 'ND100', 'ND200', 'UP370T')")
        return value

    @field_validator('point_type')
    def point_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['Bolt Contact (Bypassable)', 'Bolt Contact (Non Bypassable)', 'Duress (Silent)', 'Hold-up (Silent)', 'Hold-up (Non Silent)', 'Intrusion (Bypassable)', 'Intrusion (Non Bypassable)', 'Intrusion 24 Hour (Bypassable)', 'Intrusion 24 Hour (Non Bypassable)', 'Technical (Bypassable, Retriggerable)', 'Technical (Bypassable, Latching)', 'Technical (Bypassable, Non Latching)', 'Technical (Non Bypassable)', 'Amok (Silent)', 'Amok (Non Silent)']):
            raise ValueError("must be one of enum values ('Bolt Contact (Bypassable)', 'Bolt Contact (Non Bypassable)', 'Duress (Silent)', 'Hold-up (Silent)', 'Hold-up (Non Silent)', 'Intrusion (Bypassable)', 'Intrusion (Non Bypassable)', 'Intrusion 24 Hour (Bypassable)', 'Intrusion 24 Hour (Non Bypassable)', 'Technical (Bypassable, Retriggerable)', 'Technical (Bypassable, Latching)', 'Technical (Bypassable, Non Latching)', 'Technical (Non Bypassable)', 'Amok (Silent)', 'Amok (Non Silent)')")
        return value

    @field_validator('entry_point')
    def entry_point_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['None', 'Normal', 'Extended', 'Follower']):
            raise ValueError("must be one of enum values ('None', 'Normal', 'Extended', 'Follower')")
        return value

    @field_validator('exit_point')
    def exit_point_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['None', 'Initial Follower', 'Follower', 'Exit', 'Interrupt', 'Restart']):
            raise ValueError("must be one of enum values ('None', 'Initial Follower', 'Follower', 'Exit', 'Interrupt', 'Restart')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of LSNEMK36SConfiguration from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of LSNEMK36SConfiguration from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "supportsDelayedReporting": obj.get("supportsDelayedReporting"),
            "walktestCategory": obj.get("walktestCategory"),
            "chimeModeCapable": obj.get("chimeModeCapable"),
            "delayForAlarmDetection": obj.get("delayForAlarmDetection"),
            "entryPoint": obj.get("entryPoint"),
            "exitPoint": obj.get("exitPoint"),
            "walktestTriggerFrequency": obj.get("walktestTriggerFrequency"),
            "reportingNumber": obj.get("reportingNumber"),
            "ignoreMagneticSupervision": obj.get("ignoreMagneticSupervision")
        })
        return _obj
