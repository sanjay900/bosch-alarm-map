...
class LSNGWLoopConfiguration(BaseModel):
    """
    LSNGWLoopConfiguration
    """ # noqa: E501
    name: StrictStr = Field(description="LSN device internal name.")
    devices: List[LSNGWLoopConfigurationDevicesInner] = Field(description="List of LSN devices connected to this LSN Loop.")
    __properties: ClassVar[List[str]] = ["name", "devices"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "devices": [LSNGWLoopConfigurationDevicesInner.from_dict(_item) for _item in obj["devices"]] if obj.get("devices") is not None else None
        })
        return _obj
