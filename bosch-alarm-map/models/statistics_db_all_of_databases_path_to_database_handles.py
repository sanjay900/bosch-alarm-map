...
class StatisticsDbAllOfDatabasesPathToDatabaseHandles(BaseModel):
    """
    SQL database handle statistics
    """ # noqa: E501
    alloc: Optional[StrictInt] = Field(default=None, description="Number of created (allocated) SQL handles")
    dealloc: Optional[StrictInt] = Field(default=None, description="Number of deleted (deallocated) SQL handles")
    usage: Optional[StrictInt] = Field(default=None, description="Number of SQL handles in use")
    __properties: ClassVar[List[str]] = ["alloc", "dealloc", "usage"]
