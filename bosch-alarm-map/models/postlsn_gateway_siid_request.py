...
POSTLSNGATEWAYSIIDREQUEST_ONE_OF_SCHEMAS = ["DeviceEnableDisable", "DeviceFwVersion"]

class PostlsnGatewaySIIDRequest(BaseModel):
    """
    ...
    oneof_schema_2_validator: Optional[DeviceFwVersion] = None
    actual_instance: Optional[Union[DeviceEnableDisable, DeviceFwVersion]] = None
    one_of_schemas: Set[str] = { "DeviceEnableDisable", "DeviceFwVersion" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PostlsnGatewaySIIDRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PostlsnGatewaySIIDRequest with oneOf schemas: DeviceEnableDisable, DeviceFwVersion. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PostlsnGatewaySIIDRequest with oneOf schemas: DeviceEnableDisable, DeviceFwVersion. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PostlsnGatewaySIIDRequest with oneOf schemas: DeviceEnableDisable, DeviceFwVersion. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostlsnGatewaySIIDRequest with oneOf schemas: DeviceEnableDisable, DeviceFwVersion. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeviceEnableDisable, DeviceFwVersion]]:
