...
POSTINTERPROGRAMSIIDREQUEST_ONE_OF_SCHEMAS = ["DeviceActivateDeactivate", "InternalProgramIpArmingInfo"]

class PostInterprogramSIIDRequest(BaseModel):
    """
    ...
    # data type: InternalProgramIpArmingInfo
    oneof_schema_2_validator: Optional[InternalProgramIpArmingInfo] = None
    actual_instance: Optional[Union[DeviceActivateDeactivate, InternalProgramIpArmingInfo]] = None
    one_of_schemas: Set[str] = { "DeviceActivateDeactivate", "InternalProgramIpArmingInfo" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PostInterprogramSIIDRequest.model_construct()
        error_messages = []
        match = 0
        ...
        if not isinstance(v, DeviceActivateDeactivate):
            ...
            match += 1
        # validate data type: InternalProgramIpArmingInfo
        if not isinstance(v, InternalProgramIpArmingInfo):
            error_messages.append(f"Error! Input type `{type(v)}` is not `InternalProgramIpArmingInfo`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PostInterprogramSIIDRequest with oneOf schemas: DeviceActivateDeactivate, InternalProgramIpArmingInfo. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PostInterprogramSIIDRequest with oneOf schemas: DeviceActivateDeactivate, InternalProgramIpArmingInfo. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into InternalProgramIpArmingInfo
        try:
            instance.actual_instance = InternalProgramIpArmingInfo.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PostInterprogramSIIDRequest with oneOf schemas: DeviceActivateDeactivate, InternalProgramIpArmingInfo. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PostInterprogramSIIDRequest with oneOf schemas: DeviceActivateDeactivate, InternalProgramIpArmingInfo. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeviceActivateDeactivate, InternalProgramIpArmingInfo]]:
