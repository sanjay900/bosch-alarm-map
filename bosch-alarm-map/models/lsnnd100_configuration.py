...
LSNND100CONFIGURATION_ANY_OF_SCHEMAS = ["LSNLoopNonSilentHoldupPointPost", "LSNLoopSilentDuressPointPost", "LSNLoopSilentHoldupPointPost", "LSNND100ConfigurationAnyOf"]

class LSNND100Configuration(BaseModel):
    """
    ...
    # data type: LSNND100ConfigurationAnyOf
    anyof_schema_4_validator: Optional[LSNND100ConfigurationAnyOf] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[LSNLoopNonSilentHoldupPointPost, LSNLoopSilentDuressPointPost, LSNLoopSilentHoldupPointPost, LSNND100ConfigurationAnyOf]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "LSNLoopNonSilentHoldupPointPost", "LSNLoopSilentDuressPointPost", "LSNLoopSilentHoldupPointPost", "LSNND100ConfigurationAnyOf" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = LSNND100Configuration.model_construct()
        error_messages = []
        ...
        # validate data type: LSNND100ConfigurationAnyOf
        if not isinstance(v, LSNND100ConfigurationAnyOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `LSNND100ConfigurationAnyOf`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in LSNND100Configuration with anyOf schemas: LSNLoopNonSilentHoldupPointPost, LSNLoopSilentDuressPointPost, LSNLoopSilentHoldupPointPost, LSNND100ConfigurationAnyOf. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_4_validator: Optional[LSNND100ConfigurationAnyOf] = None
        try:
            instance.actual_instance = LSNND100ConfigurationAnyOf.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into LSNND100Configuration with anyOf schemas: LSNLoopNonSilentHoldupPointPost, LSNLoopSilentDuressPointPost, LSNLoopSilentHoldupPointPost, LSNND100ConfigurationAnyOf. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], LSNLoopNonSilentHoldupPointPost, LSNLoopSilentDuressPointPost, LSNLoopSilentHoldupPointPost, LSNND100ConfigurationAnyOf]]:
