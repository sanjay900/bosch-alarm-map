...
class StatisticsOii(BaseModel):
    """
    StatisticsOii
    """ # noqa: E501
    start: Optional[StrictStr] = Field(default=None, description="Start date")
    last: Optional[StrictStr] = Field(default=None, description="Last update date")
    duration: Optional[StrictStr] = Field(default=None, description="Amount of time between start of statistics and last statistic update")
    interval: Optional[StrictStr] = Field(default=None, description="Frequency interval, on average how often statistic is appended")
    counter: Optional[StrictInt] = Field(default=None, description="Update counter, number of changes made to statistic module")
    requests: Optional[StrictInt] = Field(default=None, description="Number of REST-API requests")
    responses: Optional[StrictInt] = Field(default=None, description="Number of REST-API responses")
    positive: Optional[StrictInt] = Field(default=None, description="Number of positive REST-API responses")
    positive: Optional[StrictInt] = Field(default=None, description="Percent of positive REST-API responses", alias="positive%")
    negative: Optional[StrictInt] = Field(default=None, description="Number of negative REST-API responses")
    negative: Optional[StrictInt] = Field(default=None, description="Percent of negative REST-API responses", alias="negative%")
    positive_responses: Optional[List[StrictStr]] = Field(default=None, description="List of used REST-API endpoints that returned positive response with:   - endpoint URL   - request type   - request command (if available)   - number of responses   - minimum time to respond in milliseconds   - average time to respond in milliseconds   - maximum time to respond in milliseconds ", alias="positiveResponses")
    negative_responses: Optional[List[StrictStr]] = Field(default=None, description="List of used REST-API endpoints that returned negative response with:   - endpoint URL   - request type   - http code   - count of responses ", alias="negativeResponses")
    clients: Optional[List[StatisticsOiiAllOfClientsInner]] = Field(default=None, description="List of REST-API clients (client name format: USERID@IP, e.g. \"005@127.0.0.1\") with client statistic:   - date and time of the first request received   - date and time of the last request received   - time duration between last request and first   - interval, on average how often statistic is appended   - delay between requests (minimum, average, maximum)   - number of requests   - number of responses sent   - number of positive responses sent   - number of negative responses sent ")
    __properties: ClassVar[List[str]] = ["start", "last", "duration", "interval", "counter", "requests", "responses", "positive", "positive%", "negative", "negative%", "positiveResponses", "negativeResponses", "clients"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "negative%": obj.get("negative%"),
            "positiveResponses": obj.get("positiveResponses"),
            "negativeResponses": obj.get("negativeResponses"),
            "clients": [StatisticsOiiAllOfClientsInner.from_dict(_item) for _item in obj["clients"]] if obj.get("clients") is not None else None
        })
        return _obj
