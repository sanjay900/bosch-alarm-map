...
class LSNGWConfigurationCreatePost(BaseModel):
    """
    LSNGWConfigurationCreatePost
    """ # noqa: E501
    connection: StrictStr = Field(description="Specifies the port where the device is connected to.")
    name: StrictStr = Field(description="LSN GW device internal name.")
    area: StrictStr = Field(description="Area of the device.")
    serial_number: StrictStr = Field(description="Product ID (serial number) of the device.", alias="serialNumber")
    ...
    bus_mode: Optional[StrictStr] = Field(default=None, description="Bus mode of the device.", alias="busMode")
    ...
    consider_device_missing_as_alarm: Optional[StrictBool] = Field(default=None, description="If set to true, missing device will be considered as alarm.", alias="considerDeviceMissingAsAlarm")
    consider_loop_failure_as_tamper_in_armed_condition: Optional[StrictBool] = Field(default=None, description="If set to true, any loop error will be considered as tamper alarm.", alias="considerLoopFailureAsTamperInArmedCondition")
    consider_loop_failure_as_tamper_in_disarmed_condition: Optional[StrictBool] = Field(default=None, description="If set to true, any loop error will be considered as tamper alarm.", alias="considerLoopFailureAsTamperInDisarmedCondition")
    loop_failure_indication: Optional[LSNLoopFailureIndicationProperty] = Field(default=None, alias="loopFailureIndication")
    aux: List[LSNGWAUXConfiguration] = Field(description="List of AUX power devices related to this LSN GW.")
    loop: LSNGWLoopConfiguration = Field(description="List of the loop devices connected to this LSN GW.")
    cmd: StrictStr = Field(description="POST request command.", alias="@cmd")
    __properties: ClassVar[List[str]] = ["connection", "name", "area", "serialNumber", "reportingNumber", "busMode", "supportsDelayedReporting", "considerDeviceMissingAsAlarm", "considerLoopFailureAsTamperInArmedCondition", "considerLoopFailureAsTamperInDisarmedCondition", "loopFailureIndication", "aux", "loop", "@cmd"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in aux (list)
        _items = []
        if self.aux:
            for _item_aux in self.aux:
                if _item_aux:
                    _items.append(_item_aux.to_dict())
            _dict['aux'] = _items
        # override the default output from pydantic by calling `to_dict()` of loop
        if self.loop:
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "busMode": obj.get("busMode"),
            "supportsDelayedReporting": obj.get("supportsDelayedReporting"),
            "considerDeviceMissingAsAlarm": obj.get("considerDeviceMissingAsAlarm"),
            "considerLoopFailureAsTamperInArmedCondition": obj.get("considerLoopFailureAsTamperInArmedCondition"),
            "considerLoopFailureAsTamperInDisarmedCondition": obj.get("considerLoopFailureAsTamperInDisarmedCondition"),
            "loopFailureIndication": obj.get("loopFailureIndication"),
            "aux": [LSNGWAUXConfiguration.from_dict(_item) for _item in obj["aux"]] if obj.get("aux") is not None else None,
            "loop": LSNGWLoopConfiguration.from_dict(obj["loop"]) if obj.get("loop") is not None else None,
            "@cmd": obj.get("@cmd")
        })
        return _obj
