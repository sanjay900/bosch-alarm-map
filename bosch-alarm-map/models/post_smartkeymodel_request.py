...
class PostSmartkeymodelRequest(BaseModel):
    """
    PostSmartkeymodelRequest
    """ # noqa: E501
    cmd: StrictStr = Field(alias="@cmd")
    smartkey_model_id: StrictStr = Field(description="Unique name of a Smartkey Model. The name is used to identify the item on the MAP system. The following charaters are forbidden by the choose of the identifier name: \" @ ;", alias="smartkeyModelID")
    area_scope_list: Optional[List[StrictStr]] = Field(default=None, description="List of areas in the scope of the Smartkey profile", alias="areaScopeList")
    arm_authority: Optional[StrictStr] = Field(default=None, description="Arm Authority of a Smartkey user during the configured time model", alias="armAuthority")
    disarm_authority: Optional[StrictStr] = Field(default=None, description="Disarm authority of a Smartkey user during the configured time model", alias="disarmAuthority")
    time_model_used_for_disarming: Optional[StrictStr] = Field(default=None, description="Related time model for disarming. Set no time model if the Smartkey model is not restricted by a time model.", alias="timeModelUsedForDisarming")
    ...
    __properties: ClassVar[List[str]] = ["@cmd", "smartkeyModelID", "areaScopeList", "armAuthority", "disarmAuthority", "timeModelUsedForDisarming", "smartkeyModelSyncID"]
    ...
    @field_validator('arm_authority')
    def arm_authority_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['None', 'CanArmAnyTime']):
            raise ValueError("must be one of enum values ('None', 'CanArmAnyTime')")
        return value

    @field_validator('disarm_authority')
    def disarm_authority_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['None', 'CanOnlyUseDoorOpener', 'CanDisarm', 'CanDisarmOnlyIfInArm', 'CanDisarmExceptIfInArm']):
            raise ValueError("must be one of enum values ('None', 'CanOnlyUseDoorOpener', 'CanDisarm', 'CanDisarmOnlyIfInArm', 'CanDisarmExceptIfInArm')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@cmd": obj.get("@cmd"),
            "smartkeyModelID": obj.get("smartkeyModelID"),
            "areaScopeList": obj.get("areaScopeList"),
            "armAuthority": obj.get("armAuthority"),
            "disarmAuthority": obj.get("disarmAuthority"),
            "timeModelUsedForDisarming": obj.get("timeModelUsedForDisarming"),
