...
GETSUPERVISEDCONNSSIID200RESPONSE_ANY_OF_SCHEMAS = ["SupervisedConns", "SupervisedIPC"]

class GetSupervisedConnsSIID200Response(BaseModel):
    """
    GetSupervisedConnsSIID200Response
    """

    # data type: SupervisedConns
    anyof_schema_1_validator: Optional[SupervisedConns] = None
    # data type: SupervisedIPC
    anyof_schema_2_validator: Optional[SupervisedIPC] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[SupervisedConns, SupervisedIPC]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "SupervisedConns", "SupervisedIPC" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetSupervisedConnsSIID200Response.model_construct()
        error_messages = []
        # validate data type: SupervisedConns
        if not isinstance(v, SupervisedConns):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SupervisedConns`")
        else:
            return v

        # validate data type: SupervisedIPC
        if not isinstance(v, SupervisedIPC):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SupervisedIPC`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetSupervisedConnsSIID200Response with anyOf schemas: SupervisedConns, SupervisedIPC. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[SupervisedConns] = None
        try:
            instance.actual_instance = SupervisedConns.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[SupervisedIPC] = None
        try:
            instance.actual_instance = SupervisedIPC.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetSupervisedConnsSIID200Response with anyOf schemas: SupervisedConns, SupervisedIPC. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], SupervisedConns, SupervisedIPC]]:
