...
class Walktest(BaseModel):
    ...
    user: Optional[StrictStr] = Field(default=None, description="Id of the user that started the walktest")
    interface: Optional[StrictStr] = Field(default=None, description="Interface that was used to start the walktest")
    wt: Optional[List[WalktestWtInner]] = Field(default=None, description="List of tuples of “area” and “wtStatus”")
    __properties: ClassVar[List[str]] = ["@type", "@self", "user", "interface", "wt"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in wt (list)
        _items = []
        if self.wt:
            for _item_wt in self.wt:
                if _item_wt:
                    _items.append(_item_wt.to_dict())
            _dict['wt'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "wt": [WalktestWtInner.from_dict(_item) for _item in obj["wt"]] if obj.get("wt") is not None else None
        })
        return _obj
