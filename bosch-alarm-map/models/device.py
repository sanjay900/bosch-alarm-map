...
class Device(BaseModel):
    ...
    op_state: Optional[DeviceOpState] = Field(default=None, alias="opState")
    incs: Optional[List[StrictStr]] = Field(default=None, description="A list of incidents that relate to this device. In case the opState is MALFUNCTION the incident will give more detailed information about the error condition the device is in.")
    enabled: Optional[StrictBool] = Field(default=None, description="True if device is currently enabled, otherwise false.")
    active: Optional[StrictBool] = Field(default=None, description="True if device is currently activated, otherwise false.")
    on: Optional[StrictBool] = Field(default=None, description="True if output is on")
    walktest: Optional[DeviceWalktest] = None
    bypassed: Optional[StrictBool] = Field(default=None, description="True if device is currently bypassed, otherwise false")
    activated: Optional[StrictBool] = Field(default=None, description="True if device is currently activated, otherwise false.")
    user_id: Optional[StrictStr] = Field(default=None, description="The user ID of the user that is currently logged into the Keypad. In case no user is logged in, the Empty string “” will be set. Thus, a client that is subscribed for events for a Keypad will receive a state change event including timestamp when a user logs in or off.  ", alias="userID")
    locked: Optional[StrictBool] = Field(default=None, description="True if arming device is locked (resulting in the door being locked)")
    update: Optional[DeviceUpdate] = None
    usable: Optional[StrictBool] = Field(default=None, description="True if the arming device can be used to arm/disarm the area")
    __properties: ClassVar[List[str]] = ["@type", "@self", "opState", "incs", "enabled", "active", "on", "walktest", "bypassed", "activated", "userID", "locked", "update", "usable"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "@type": obj.get("@type"),
            "@self": obj.get("@self"),
            "opState": obj.get("opState"),
            "incs": obj.get("incs"),
            "enabled": obj.get("enabled"),
            "active": obj.get("active"),
            "on": obj.get("on"),
            "walktest": obj.get("walktest"),
            "bypassed": obj.get("bypassed"),
