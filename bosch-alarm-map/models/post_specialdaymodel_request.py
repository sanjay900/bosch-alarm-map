...
class PostSpecialdaymodelRequest(BaseModel):
    """
    PostSpecialdaymodelRequest
    """ # noqa: E501
    cmd: StrictStr = Field(alias="@cmd")
    special_day_model_id: StrictStr = Field(description="Unique Name of the Special day. The name is used to identify the item on the MAP System. The following characters are forbidden in identifier name: \" @ ; ", alias="specialDayModelID")
    day_model_id: Optional[StrictStr] = Field(default=None, description="Unique name (ID) of an existing Access Model", alias="dayModelID")
    priority: Optional[Annotated[int, Field(le=100, strict=True, ge=1)]] = Field(default=None, description="If more than one time model is configured at the same date, the time model with the highest priority will be executed (1 = highest, 100 lowest)")
    next_occurrence: Optional[StrictStr] = Field(default=None, description="Yearly repeating Date of the Special day. The date of nextOccurrence must be today or in the future.", alias="nextOccurrence")
    repeat_policy: Optional[StrictStr] = Field(default=None, description="Final date, up to the specialmodel is guilty. It is optional in custom and easterBased. Date must be today or in the future.", alias="repeatPolicy")
    custom_rule: Optional[StrictStr] = Field(default=None, description="The Custom rules are constructed with 2 or 3 words. The words are separated with a space. Allowed parameters for index, day and month: <br> The index value can be: <br>   - FIRST   - SECOND   - THIRD   - FOURTH   - FIFTH  the day value can be:<br>   - MONDAY   - TUESDAY   - WEDNESDAY   - THURSDAY   - FRIDAY   - SATURDAY   - SUNDAY  The month value can by: <br> - JANUARY - FEBRUARY - MARCH - APRIL - MAY - JUNE - JULY - AUGUST - SEPTEMBER - OCTOBER - NOVEMBER - DECEMBER  #### Custom rules 2 words:  **Index Day** or <br> **Index Month** <br>  Example *2 word rule* (each week, every month)<br> \"SECOND FRIDAY\"<br> That means that this special day occurs on every second Friday of every month.<br>  Example *2 word rule* (particular month, every year)<br> \"FIRST JUNE\"<br> That means that this special day occurs on every first June of every year.<br>  #### Custom rules 3 words:  **INDEX DAY MONTH** Example 3 word rule (defined day, particular month, every year<br> \"FIRST SUNDAY MARCH\"<br> That means that this special day occurs on first Sunday of each March.<br> ", alias="customRule")
    easter_offset: Optional[StrictStr] = Field(default=None, description="The vaule is required only for easterBase.", alias="easterOffset")
    repeat_until: Optional[StrictStr] = Field(default=None, description="The date of nextOccurrence must be today or in the future.", alias="repeatUntil")
    ...
    __properties: ClassVar[List[str]] = ["@cmd", "specialDayModelID", "dayModelID", "priority", "nextOccurrence", "repeatPolicy", "customRule", "easterOffset", "repeatUntil", "specialDayModelSyncID"]

    @field_validator('cmd')
    def cmd_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CREATE', 'MODIFY', 'DELETE']):
            raise ValueError("must be one of enum values ('CREATE', 'MODIFY', 'DELETE')")
        return value

    @field_validator('repeat_policy')
    def repeat_policy_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['annually', 'monthly', 'easterBased', 'custom', 'no']):
            raise ValueError("must be one of enum values ('annually', 'monthly', 'easterBased', 'custom', 'no')")
        return value

    @field_validator('easter_offset')
    def easter_offset_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['-2', '0', '1', '39', '49', '50', '60']):
            raise ValueError("must be one of enum values ('-2', '0', '1', '39', '49', '50', '60')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PostSpecialdaymodelRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PostSpecialdaymodelRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "@cmd": obj.get("@cmd"),
            "specialDayModelID": obj.get("specialDayModelID"),
            "dayModelID": obj.get("dayModelID"),
            "priority": obj.get("priority"),
            "nextOccurrence": obj.get("nextOccurrence"),
            "repeatPolicy": obj.get("repeatPolicy"),
            "customRule": obj.get("customRule"),
            "easterOffset": obj.get("easterOffset"),
            "repeatUntil": obj.get("repeatUntil"),
