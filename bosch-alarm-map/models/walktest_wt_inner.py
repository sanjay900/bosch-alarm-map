...
class WalktestWtInner(BaseModel):
    """
    ...
    """ # noqa: E501
    area: Optional[StrictStr] = Field(default=None, description="URL to the area that is part of this walktest")
    wt_status: Optional[StrictStr] = Field(default=None, description="Walktest status of the area. Possible values are: - PENDING: walktest requested but not started - STARTED: walktest active - FAILED: walktest requested but area did not start test, e.g. because of area being armed ", alias="wtStatus")
    __properties: ClassVar[List[str]] = ["area", "wtStatus"]

    @field_validator('wt_status')
    def wt_status_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['PENDING', 'STARTED', 'FAILED']):
            raise ValueError("must be one of enum values ('PENDING', 'STARTED', 'FAILED')")
        return value
